...
float dmp::Timer::time() const
{
   ...
   // TODO: Jan 3: delete this comment block if not important
   // --------------------------------------------------------------------------
   // now is now set to be the base time plus the delta of the previous
   // effective now minus how long we spent paused
   // now = mBaseTime + (now - mPausedTime);
   // auto final = now - mBaseTime;
   // --------------------------------------------------------------------------

   // The final elapsed time is the difference between the effective "now" and
   // how long we spent paused.
